#!/usr/bin/env node
/**
 * Pair 26 — CURRENT SESSION exchange reconciliation.
 *   Window: pair.sessionStartedAt (else botStartedAt) → Date.now()
 *   Same exchange endpoints as eth26ExchangeReport.js
 *
 * DB fields used: sessionStartedAt, sessionStartBalance, botStartedAt,
 * tradeAccountA, agentName, lastStopReason.
 */
'use strict';
...
(async () => {
  ...
  const sessMs = pair.sessionStartedAt ? new Date(pair.sessionStartedAt).getTime() : null;
  const startMs = sessMs != null && Number.isFinite(sessMs) ? sessMs : new Date(pair.botStartedAt).getTime();
  const endMs   = Date.now();
  const dbSessionEquity = pair.sessionStartBalance != null ? parseFloat(pair.sessionStartBalance) : null;

  console.log(`\nPair ${PAIR_ID} — CURRENT SESSION — pulling exchange data`);
  console.log(`  Window: ${fmtTs(startMs)}  →  ${fmtTs(endMs)}`);
  ...
  L.push(HR);
  L.push(`  ETH BOT  |  PAIR ${PAIR_ID}  |  CURRENT SESSION  |  ${pair.agentName || ''}`);
  L.push(`  Account      : ${pair.tradeAccountA}`);
  L.push(`  Instrument   : ${INSTRUMENT}`);
  L.push(`  Session      : ${fmtTs(startMs)}  →  ${fmtTs(endMs)}  (${fmtDur(endMs-startMs)})`);
  L.push(`  Session anchor: ${pair.sessionStartedAt ? 'sessionStartedAt (DB)' : 'botStartedAt (fallback)'}`);
  ...
  /* 0. DB vs exchange equity (session) */
  L.push('');
  L.push('  0.  SESSION ANCHORS  (DB ↔ exchange reconciliation)');
  L.push(hr);
  if (dbSessionEquity != null && Number.isFinite(dbSessionEquity)) {
    const liveEq = parseFloat(currentEquity);
    const drift = liveEq - dbSessionEquity;
    L.push(`  DB sessionStartBalance (equity at last enable): ${dbSessionEquity.toFixed(6)} ETH  (~$${(dbSessionEquity * ethIdx).toFixed(2)})`);
    L.push(`  Live equity now (get_account_summary)         : ${liveEq.toFixed(6)} ETH  (~$${(liveEq * ethIdx).toFixed(2)})`);
    L.push(`  Δ equity (live − DB session anchor)           : ${sgn(drift)}${drift.toFixed(6)} ETH  (~$${(drift * ethIdx).toFixed(2)})`);
    L.push(`  Note: Δ includes ALL account PnL (options, other instruments), not only ETH-PERP fills below.`);
  } else {
    L.push(`  DB sessionStartBalance: (null) — enable path may not have stamped yet`);
  }
  L.push('');
  ...
  const out = path.join(__dirname, '..', 'reports', `eth_pair26_current_session_exchange_${ts}.txt`);
