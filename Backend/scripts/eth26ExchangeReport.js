#!/usr/bin/env node
/**
 * Pair 26 — comprehensive analysis computed ENTIRELY from exchange data.
 *   - get_user_trades_by_instrument_and_time  (ETH-PERPETUAL fills only)
 *   - get_transaction_log                     (settlement / delivery / fees)
 *   - get_account_summary                     (current equity / balance)
 *   - get_positions                           (current open position)
 *
 * Nothing is taken from the internal DB except pair.botStartedAt /
 * pair.lastDisabledAt / pair.tradeAccountA / pair.tradeLegA (identifiers).
 */
'use strict';
...
(async () => {
  ...
  console.log(`\nPair ${PAIR_ID} — pulling exchange data`);
  console.log(`  Window: ${fmtTs(startMs)}  →  ${fmtTs(endMs)}`);
  console.log(`  Instrument: ${INSTRUMENT}\n`);

  const [fills, txLog, positions, summary, idx] = await Promise.all([
  ...
  L.push(`    • /get_account_summary (${CCY})  /get_positions (future)`);
  L.push(HR);
  ...
  L.push(`  Avg hold           : ${fmtDur(totalHold/n)}`);
  ...
  L.push(`  Expectancy / trip  : ${(realizedEth/n).toFixed(6)} ETH   (~$${((realizedEth/n)*ethIdx).toFixed(2)})`);
  L.push('');

  /* 6. hold-time distribution */
  const holds = trips.map(t => t.hold).sort((a,b)=>a-b);
  const p = q => holds[Math.min(holds.length-1, Math.floor(holds.length*q))];
  L.push('  6.  HOLD TIME DISTRIBUTION');
  L.push(hr);
  L.push(`  min  : ${fmtDur(holds[0]||0)}`);
  L.push(`  p25  : ${fmtDur(p(0.25))}`);
  L.push(`  p50  : ${fmtDur(p(0.5))}`);
  L.push(`  p75  : ${fmtDur(p(0.75))}`);
  L.push(`  p90  : ${fmtDur(p(0.9))}`);
  L.push(`  max  : ${fmtDur(holds[holds.length-1]||0)}`);
  L.push('');
  ...
  const out = path.join(__dirname, '..', 'reports', `eth_pair26_exchange_report_${ts}.txt`);
