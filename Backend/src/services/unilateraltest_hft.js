...
class UnilateralExecutor {
  constructor() {
    ...
    /** @type {Record<string, object>} keyed paper engines (e.g. SOL, ETH, BTC) */
    this._papers = {};
  }
  ...
  initPaperTrading(pairId, cfg = {}) {
    const pt = {
      ...
      _csvFile:       cfg.csvFile != null ? cfg.csvFile : null,
      persistDb:      cfg.persistDb === true,
      ...
      /** When max entry ladder rung is tiny (e.g. XRP), skip A–S askSpread vs signal gate — quotes sit above signal too often. */
      _microLadder:   false,
      // false (default): SL when |deribitMid − entryPrice| ≥ sl (USD). true: legacy spread SL.
      stopLossOnSpread: cfg.stopLossOnSpread === true,
      // true: price SL only after deribitMid leaves the entry zone band [low, high) from open.
      stopLossRequireZoneExit: cfg.stopLossRequireZoneExit === true,
      persistModel:   cfg.persistModel || 'sol', // 'sol' | 'cross'
      pairKey:        pairId,
    };
    ...
    const lv = (pt.entryLevels || []).map(Number).filter(Number.isFinite);
    pt._microLadder = lv.length > 0 && Math.max(...lv) < 0.025;

    this._papers[pairId] = pt;
    const slGate = pt.stopLossRequireZoneExit ? ' SL=after zone exit' : '';
    this._paperLog(pt, `Paper trading initialised | pair=${pairId} capital=$${pt.capital}${slGate}`);
    return pt;
  }

  _paperLog(pt, msg) {
    if (!pt) return;
    const line = `[${new Date().toISOString()}] [${pt.pairKey || '?'}] ${msg}`;
  ...
  _persistPaperEntry(pt, pos, snap) {
    if (!pt.persistDb || !pt.sessionId) return;
    if (pt.persistModel === 'cross') {
      const { CrossPaperTrade } = require('../models');
      CrossPaperTrade.create({
        pairKey:      pt.pairKey,
        ...
        deribitMid:   snap.price,
        hlMid:        snap.hyperMid ?? null,
        asAskSpread:  snap.asAskSpread,
        zScore:       snap.zScore,
        regime:       snap.regime,
        inventoryAfter: snap.inventoryAfter,
        equityAfter:    snap.equityAfter,
        pnlUsd:       null,
        holdSec:      null,
        peakNarrow:   null,
        exitReason:   null,
        deribitOrderId: null,
        eventAt:      new Date(snap.now),
      }).then((row) => { pos.paperDbId = row.id; })
        .catch((e) => console.error('[CrossPaper DB ENTRY]', e.message));
      return;
    }
    const { SolPaperTrade } = require('../models');
    SolPaperTrade.create({
      sessionId:    pt.sessionId,
      botId:        null,
      type:         'ENTRY',
      entryRowId:   null,
      ...
      .catch((e) => console.error('[PaperTrade DB ENTRY]', e.message));
  }

  _persistPaperExit(pt, pos, snap) {
    if (!pt.persistDb || !pt.sessionId) return;
    if (pt.persistModel === 'cross') {
      const { CrossPaperTrade } = require('../models');
      CrossPaperTrade.create({
        ...
        equityAfter:    snap.equityAfter,
        pnlUsd:       snap.pnlUsd,
        holdSec:      snap.holdSec,
        peakNarrow:   snap.peakNarrow,
        exitReason:   snap.exitReason,
        deribitOrderId: null,
        eventAt:      new Date(snap.now),
      }).catch((e) => console.error('[CrossPaper DB EXIT]', e.message));
      return;
    }
    const { SolPaperTrade } = require('../models');
    SolPaperTrade.create({
      ...
      deribitOrderId: null,
      eventAt:      new Date(snap.now),
    }).catch((e) => console.error('[PaperTrade DB EXIT]', e.message));
  }

  /**
   * @param {string|object} pairKeyOrSpread - pair key (ETH, SOL, …) or legacy spread payload
   * @param {object} [spreadData] - when first arg is pair key
   */
  onPaperSpreadUpdate(pairKeyOrSpread, spreadData) {
    let pairKey = 'paper_sol';
    let data = pairKeyOrSpread;
    if (typeof pairKeyOrSpread === 'string' && spreadData && typeof spreadData === 'object') {
      pairKey = pairKeyOrSpread;
      data = spreadData;
    }
    const pt = this._papers[pairKey];
    if (!pt || !pt.enabled) return;

    const log = (m) => this._paperLog(pt, m);
    const csv = (row) => this._paperCsv(pt, row);
    ...
    if (pt.zoneGrid && price > 0) {
      ...
      if (zone) {
        if (!pt._activeZone || pt._activeZone.id !== zone.id) {
          pt._activeZone = zone;
          log(`Zone → ${zone.id} ($${zone.low.toFixed(2)}-$${zone.high.toFixed(2)}) price=$${price.toFixed(2)}`);
        }
        zoneCfg = pt.zones[zone.index] || null;
      } else if (pt._activeZone) {
        pt._activeZone = null;
        log(`Price $${price.toFixed(2)} outside zone range`);
      }
    }
    ...
    if (pt.regime !== prevRegime) {
      log(`Regime ${prevRegime} → ${pt.regime} | hurst=${hurst?.toFixed(4)} bo=${boResult.triggers} triggers`);
    }
    ...
    for (const pos of pt.positions) {
      ...
      let slHit = false;
      if (holdMs >= pt.minHoldMs) {
        if (pt.stopLossOnSpread) {
          slHit = (signalSpread - pos.fillSpread) >= pos.sl;
        } else {
          const adversePx = Math.abs(price - pos.entryPrice) >= pos.sl;
          if (pt.stopLossRequireZoneExit && pos.entryZoneId && pt.zoneGrid) {
            const zNow = pt.zoneGrid.getZone(price);
            const stillInEntryZone = zNow && zNow.id === pos.entryZoneId;
            slHit = !stillInEntryZone && adversePx;
          } else {
            slHit = adversePx;
          }
        }
      ...
      if (exitReason) {
        ...
        if (exitZone && pos.lifeId != null) {
          if (exitReason === 'stop_loss') {
            ...
            log(`LIFE KILLED | zone=${pos.zoneId} life=${pos.lifeId} (SL) gone=${isZoneGone(exitZone)}`);
          } else {
            releaseLife(exitZone, pos.lifeId);
            log(`LIFE RELEASED | zone=${pos.zoneId} life=${pos.lifeId} (${exitReason})`);
          }
        }
        ...
        this._persistPaperExit(pt, pos, {
          signalSpread,
          price,
          hyperMid,
          zScore,
          regime: pt.regime,
          inventoryAfter: pt.inventory,
          equityAfter: pt.equity,
          pnlUsd,
          holdSec: holdMs / 1000,
    ...
    // Micro ladders (XRP-scale): allow entries in BO — otherwise z-score spikes block all day.
    if (pt.regime === REGIME_BO && !pt._microLadder) { pt.prevSpread = signalSpread; return; }
    ...
    if (reg === 'trending' && !pt._microLadder) { pt.prevSpread = signalSpread; return; }

    // A-S ask entry: only enter when spread > our ask quote (skipped for micro ladders: XRP-scale)
    if (!pt._microLadder && signalSpread < quotes.askSpread) { pt.prevSpread = signalSpread; return; }
    ...
    for (let i = levels.length - 1; i >= 0; i--) {
      ...
      if (pt.prevSpread < lvl && signalSpread >= lvl) {
        ...
        if (!life) {
          log(`ENTRY skip [no free life] zone=${pt._activeZone.id} lvl=${i+1}`);
          continue;
        }

        const lvlQty = (zoneCfg.levelQtys && zoneCfg.levelQtys[i] != null)
          ? zoneCfg.levelQtys[i]
          : (zoneCfg.qty || 10);
        const ez = pt._activeZone;
        const pos = {
          id:             posId,
          status:         'open',
          ...
          entryZoneId:    ez != null ? ez.id : null,
          peakNarrowing:  0,
        ...
        this._persistPaperEntry(pt, pos, {
          signalSpread,
          price,
          hyperMid,
  ...
  getPaperState(pairKey = 'paper_sol') {
    const pt = this._papers[pairKey];
    if (!pt) return null;
    const openPos = pt.positions.filter(p => p.status === 'open');
    return {
      sessionId:     pt.sessionId || null,
      enabled:       pt.enabled,
      regime:        pt.regime,
      ...
      positions:     openPos.map(p => ({
        zoneId: p.zoneId, level: p.gridLevel, gridLevel: p.gridLevel, qty: p.qty, lifeId: p.lifeId,
      ...
      pairKey,
      persistModel: pt.persistModel || 'sol',
    };
  }

  /** @returns {string[]} */
  listPaperPairKeys() {
    return Object.keys(this._papers || {});
  }

  stopAllPaperPairs() {
    const keys = this.listPaperPairKeys();
    let last = null;
    for (const k of keys) last = this.stopPaperTrading(k);
    return last;
  }

  stopPaperTrading(pairKey = 'paper_sol') {
    const pt = this._papers[pairKey];
    if (!pt) return null;
    pt.enabled = false;
    ...
    this._paperLog(pt, summary);
    delete this._papers[pairKey];
    return { summary, trades: pt.closedTrades, logLines: pt._logLines, pairKey };
  }
}
