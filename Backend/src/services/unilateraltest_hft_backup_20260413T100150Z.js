  ...
  onPaperSpreadUpdate(spreadData) {
    ...
    for (const pos of pt.positions) {
      ...
      if (exitReason) {
        ...
        this._paperLog(
          `EXIT ${exitReason} | zone=${pos.zoneId} lvl=${pos.gridLevel} ` +
    ...
    const zoneOpen = pt.positions.filter(p => p.zoneId === pt._activeZone.id).length;
    const maxPos   = zoneCfg.maxPositions || 4;
    if (zoneOpen >= maxPos) { pt.prevSpread = signalSpread; return; }
    ...
    for (let i = levels.length - 1; i >= 0; i--) {
      ...
      if (pt.prevSpread < lvl && signalSpread >= lvl) {
        ...
        this._paperLog(
          `ENTRY | zone=${pos.zoneId} lvl=${pos.gridLevel} qty=${pos.qty} ` +
  ...
  getPaperState() {
    ...
    return {
      ...
      positions:     openPos.map(p => ({
        zoneId: p.zoneId, level: p.gridLevel, qty: p.qty,
