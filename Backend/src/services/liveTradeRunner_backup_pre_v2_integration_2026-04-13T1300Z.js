 ...
 *
 * V2 maker stack (integrated, not a separate bot):
 *   Kalman fair spread, RLS adaptive edge, regime classifier (Hurst + vol ratio + inventory),
 *   inventory-aware A–S skew + recovery mode (no forced bo_inventory_reduce), maker-rebate
 *   hurdle scaling. Legacy as_bid_exit is not used.
 *
...
const DEFAULT_LIVE_START_OPTS = {
  ...
  exec: {
    ...
    /**
     * When maker rebate applies (pnlFeeBpsPerLeg < 0), scale fee hurdle by this factor (<1 = easier exits/entries).
     * “Maker rebate optimizer” — conservative default 0.88.
     */
    makerRebateHurdleScale: 0.88,
  },
};

/** V2 signal / edge stack — merged with opts.v2 in start(). */
const DEFAULT_V2_OPTS = {
  enabled: true,
  rlsLambda: 0.995,
  rlsDim: 10,
  /** If > 0, require rls.predict(features) >= this before ladder ENTRY (0 = off). */
  rlsMinEdge: 0,
  kfQ: 0.0001,
  kfR: 0.01,
  hurstMRMax: 0.45,
  hurstTrendMin: 0.55,
  volRatioCaution: 1.2,
  volRatioBreakout: 1.75,
  maxInventorySol: 300,
  invRecoveryCutoff: 0.55,
  invEmergency: 0.92,
  blockEntriesTrendBreakout: true,
  blockEntriesInvEmergency: true,
  /** Extra skew intensity when recovery / inventory pressure (0..1). */
  recoverySkewGain: 0.18,
};
...
function _feeHurdleUsd(qty, mid, exec) {
  ...
  let h = mult * (bps / 10000) * q * m;
  const scale = exec.makerRebateHurdleScale;
  if (scale != null && scale > 0 && scale < 1) {
    const bpl = parseFloat(exec.pnlFeeBpsPerLeg);
    if (Number.isFinite(bpl) && bpl < 0) h *= scale;
  }
  return h;
}
...
// ─── V2: RLS edge + Kalman fair spread (1-D) ───────────────────────────────────
...
class RLS {
  constructor(dim, lambda) {
    this.dim = dim;
    this.lambda = lambda;
    this.theta = Array(dim).fill(0);
    this.P = Array(dim).fill(0).map((_, i) =>
      Array(dim).fill(0).map((__, j) => (i === j ? 100 : 0))
    );
  }

  predict(x) {
    if (!x || x.length !== this.dim) return 0;
    return _dot(this.theta, x);
  }

  update(x, y) {
    if (!x || x.length !== this.dim || !Number.isFinite(y)) return;
    const Px = this.P.map(row => _dot(row, x));
    const denom = this.lambda + _dot(x, Px);
    if (!(denom > 1e-18)) return;
    const K = Px.map(v => v / denom);
    const err = y - this.predict(x);
    this.theta = this.theta.map((t, i) => t + K[i] * err);
    this.P = this.P.map((row, i) =>
      row.map((v, j) => (v - K[i] * Px[j]) / this.lambda)
    );
  }
}

class KalmanFairSpread {
  constructor(p = {}) {
    this.q = p.kfQ ?? 0.0001;
    this.r = p.kfR ?? 0.01;
    this.x = 0;
...
function _inventoryPressure(invQty, maxSol) {
  const m = Math.max(1e-6, parseFloat(maxSol) || 300);
  return Math.min(1, Math.abs(parseFloat(invQty) || 0) / m);
}

function _classifyV2Regime(C, hurst, volRatio, invPressure) {
  const h = hurst == null ? 0.5 : hurst;
  if (h > C.hurstTrendMin || volRatio > C.volRatioBreakout) return 'TREND_BREAKOUT';
  if (invPressure > C.invRecoveryCutoff) return 'INVENTORY_RECOVERY';
  if (volRatio > C.volRatioCaution) return 'MR_CAUTION';
  if (h < C.hurstMRMax) return 'MR_PASSIVE';
  return 'VOLATILE_NOISE';
}

function _buildRlsFeatureVector(pt, o) {
  const anchor = pt.zoneGrid?.anchor ?? 84;
  const range = pt.zoneGrid?.range ?? 4;
  const midN = (o.deribitMid - anchor) / (range || 1);
  const hz = o.hurst == null ? 0.5 : o.hurst;
  return [
    1,
    o.zScore,
    o.signalSpread,
...
/** Ladder crossing + Kalman/RLS velocity (last signalSpread). */
function _bumpSpreadMemory(pt, signalSpread) {
  pt.prevSpread = signalSpread;
  if (Number.isFinite(signalSpread)) pt._prevSigForVel = signalSpread;
}
...
async function _onBookUpdate() {
  ...
  // ── V2: Kalman fair spread, vol ratio, RLS edge, regime, inventory recovery ─
  const v2 = pt.v2;
  const spreadVelSig = (pt._prevSigForVel != null && Number.isFinite(pt._prevSigForVel))
    ? (signalSpread - pt._prevSigForVel)
    : 0;
  const volRatio = pt.bo.volRatio();
  const invPressure = _inventoryPressure(pt.inventory, v2.maxInventorySol);
  const fairSpread = v2.enabled ? pt.kfSpread.update(signalSpread) : signalSpread;
  const v2Regime = v2.enabled
    ? _classifyV2Regime(v2, hurst, volRatio, invPressure)
    : 'MR_PASSIVE';
  pt.v2Regime = v2Regime;

  const openWidenAgainstShort = pt.positions.some(
    (p) => p.status === 'open' && !p._orderPending && p.entrySpread != null
      && (p.entrySpread - signalSpread) < 0
  );
  pt.recoveryMode = !!v2.enabled && (
    v2Regime === 'INVENTORY_RECOVERY'
    || invPressure >= v2.invRecoveryCutoff
    || (pt.regime === REGIME_BO && openWidenAgainstShort)
  );

  const rlsFeatures = v2.enabled
    ? _buildRlsFeatureVector(pt, {
      zScore: stats.zScore,
      signalSpread,
      spreadVelocity: spreadVelSig,
      volRatio,
      hurst,
      invPressure,
      fairSpread,
    ...
    : null;
  const v2Edge = v2.enabled && rlsFeatures ? pt.rls.predict(rlsFeatures) : 0;
  pt.v2Edge = v2Edge;
  pt.v2FairSpread = fairSpread;

  const skewDyn = { bidWidenLambda: pt.skew.bidWidenLambda, askTightenPhi: pt.skew.askTightenPhi };
  if (v2.enabled && pt.recoveryMode) {
    const g = (v2.recoverySkewGain || 0.18) * Math.max(invPressure, 0.15);
    skewDyn.bidWidenLambda *= (1 + g);
    if (pt.inventory > 0) {
      skewDyn.askTightenPhi *= Math.max(0.35, 1 - 0.55 * g);
    } else if (pt.inventory < 0) {
      skewDyn.askTightenPhi *= (1 + 0.25 * g);
    }
  } else if (v2.enabled && v2Regime === 'MR_CAUTION') {
    skewDyn.bidWidenLambda *= 1.06;
  }

  const quotes  = pt.as.quotes(stats.mean, pt.inventory, sigma, regMult, skewDyn);
  ...
  for (const pos of pt.positions) {
    ...
    // BO + spread widening vs short: recovery via skew / regime only — no forced loss exit.
    ...
    if (exitReason && !pos._orderPending) {
      ...
      _deribitLimitBuy(pos.qty).then(dOid => {
        ...
        if (v2.enabled && pos.rlsFeatures && pt.rls) {
          const y = pnlUsd / Math.max(pos.qty || 0, 0.01);
          pt.rls.update(pos.rlsFeatures, y);
        }
  ...
  // ── ENTRY checks ─────────────────────────────────────────────────────────
  if (!pt.activeZone || !zoneCfg)            { _bumpSpreadMemory(pt, signalSpread); return; }
  if (pt.regime === REGIME_BO && !pt.exec.allowBoEntries) { _bumpSpreadMemory(pt, signalSpread); return; }
  if (pt.dailyPnl <= -pt.dailyLossLimit)     { _bumpSpreadMemory(pt, signalSpread); return; }
  if (now - pt.lastEntryAt < pt.cooldownMs)  { _bumpSpreadMemory(pt, signalSpread); return; }
  if (hurst != null && hurst > pt.hurstGate) { _bumpSpreadMemory(pt, signalSpread); return; }
  if (pt.analytics.regime==='trending')      { _bumpSpreadMemory(pt, signalSpread); return; }
  if (pt.prevSpread == null)                 { _bumpSpreadMemory(pt, signalSpread); return; }

  if (isZoneGone(zoneCfg))                   { _bumpSpreadMemory(pt, signalSpread); return; }
  if (freeLives(zoneCfg).length === 0)       { _bumpSpreadMemory(pt, signalSpread); return; }
  ...
  for (let i=pt.entryLevels.length-1; i>=0; i--) {
    ...
    if (v2.enabled) {
      if (v2Regime === 'TREND_BREAKOUT' && v2.blockEntriesTrendBreakout) {
        _log(`ENTRY skip [v2] TREND_BREAKOUT lvl=${i + 1}`);
        continue;
      }
      if (invPressure >= v2.invEmergency && v2.blockEntriesInvEmergency) {
        _log(`ENTRY skip [v2] inventory emergency pressure=${invPressure.toFixed(3)} lvl=${i + 1}`);
        continue;
      }
      const minE = parseFloat(v2.rlsMinEdge);
      if (Number.isFinite(minE) && minE > 0 && v2Edge < minE) {
        _log(`ENTRY skip [v2] RLS edge=${v2Edge.toFixed(6)} < rlsMinEdge=${minE} lvl=${i + 1}`);
        continue;
      }
    }
    ...
    const pos = {
      ...
      rlsFeatures: rlsFeatures ? rlsFeatures.slice() : null,
    };
    pt.inventory  += pos.qty;
    pt.lastEntryAt = now;
  ...
  _bumpSpreadMemory(pt, signalSpread);

  // 30s status heartbeat
  if (now - _lastStatusAt > 30000) {
    _lastStatusAt = now;
    const line = `ticks=${_tickCount} regime=${pt.regime} v2=${pt.v2Regime || 'n/a'} edge=${(pt.v2Edge ?? 0).toFixed(4)} ` +
      `fairSp=${(pt.v2FairSpread ?? 0).toFixed(4)} rec=${pt.recoveryMode ? 1 : 0} inv=${pt.inventory} ` +
...
function mergeLiveOpts(overrides = {}) {
  ...
  return {
    ...
    v2:       { ...DEFAULT_V2_OPTS,       ...(overrides.v2 || {}) },
...
async function start(opts = {}) {
  ...
  const v2c = _st.v2;
  _st.rls = new RLS(v2c.rlsDim, v2c.rlsLambda);
  _st.kfSpread = new KalmanFairSpread({ kfQ: v2c.kfQ, kfR: v2c.kfR });
  ...
  console.log(`  V2 stack      : Kalman+RLS+regime skew  rebateHurdleScale=${_st.exec.makerRebateHurdleScale ?? 'n/a'}  rlsMinEdge=${_st.v2.rlsMinEdge}  maxInv=${_st.v2.maxInventorySol}`);
...
function getState() {
  ...
  return {
    ...
    exec:          _st.exec,
    v2:            _st.v2,
    v2Regime:      _st.v2Regime,
    v2Edge:        _st.v2Edge,
    v2FairSpread:  _st.v2FairSpread,
    recoveryMode:  !!_st.recoveryMode,
    pnlNetOfFees:  true,
  };
}
