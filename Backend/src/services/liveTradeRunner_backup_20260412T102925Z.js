 ...
 *
 * Order leg     : Deribit SOL_USDC-PERPETUAL ONLY (post_only maker limits; no market fallback)
...
const { AccountDetails, SolLiveTrade, SolBalanceSnapshot } = require('../models');
...
const DEFAULT_LIVE_START_OPTS = {
  ...
  zoneGrid: { anchorPrice: 84.00, range: 4, zoneCount: 4 },
  zones: [
    { qty: 1, tp: 0.05, sl: 1.0, trailPct: 0.50, maxPositions: 4 },
    { qty: 1, tp: 0.10, sl: 1.0, trailPct: 0.50, maxPositions: 4 },
    { qty: 1, tp: 0.20, sl: 1.0, trailPct: 0.50, maxPositions: 4 },
    { qty: 1, tp: 0.40, sl: 1.0, trailPct: 0.50, maxPositions: 4 },
  ],
};
 ...
 * E.g. 1.0 SOL / 4 levels → [0.3, 0.3, 0.2, 0.2] (not 0.25×4 — invalid on Deribit).
 */
...
async function _loadCredentials() {
  const row = await AccountDetails.findByPk(DERIBIT_ACCT_ID);
  if (!row) throw new Error(`Account id=${DERIBIT_ACCT_ID} not found in DB`);
...
async function _saveBalanceSnapshot(snapshotType) {
  try {
...
async function _postOnlySellFromBook(q, book, reduceOnly = false) {
  ...
  for (let bump = 0; bump <= 5; bump++) {
    const limitPx = _roundPrice(bestAsk + bump * TICK_SIZE);
    if (limitPx <= bestBid) break;
    try {
      const res = await signedRequest(
      ...
      if (msg.includes('post_only') || msg.includes('reject') || msg.includes('invalid')) {
        _log(`SELL post_only bump=${bump} @ ${limitPx}: ${(e.message || '').slice(0, 160)}`);
        continue;
      }
      throw e;
    }
...
async function _postOnlyBuyFromBook(q, book, reduceOnly = false) {
  ...
  const ro = reduceOnly ? '&reduce_only=true' : '';
  for (let bump = 0; bump <= 5; bump++) {
    const limitPx = _roundPrice(bestBid - bump * TICK_SIZE);
    if (limitPx <= 0 || limitPx >= bestAsk) break;
    try {
      const res = await signedRequest(
      ...
      const msg = (e.message || '').toLowerCase();
      if (msg.includes('post_only') || msg.includes('reject') || msg.includes('invalid')) {
        _log(`BUY  post_only bump=${bump} @ ${limitPx}: ${(e.message || '').slice(0, 160)}`);
        continue;
      }
      throw e;
    }
...
async function _onBookUpdate() {
  ...
  for (const pos of pt.positions) {
    ...
    if (exitReason && !pos._orderPending) {
      ...
      const pnlUsd   = pos.qty * (pos.entrySpread - signalSpread);
      ...
      _log(`EXIT ${exitReason} | zone=${pos.zoneId} lvl=${pos.gridLevel} qty=${pos.qty} ` +
           `entry=$${pos.entrySpread.toFixed(4)} exit=$${snapSpread.toFixed(4)} pnl≈$${pnlUsd.toFixed(4)}`);
  ...
  const zoneOpen = pt.positions.filter(p=>p.zoneId===pt.activeZone.id).length;
  if (zoneOpen >= (zoneCfg.maxPositions||4)) { pt.prevSpread=signalSpread; return; }
  ...
  for (let i=pt.entryLevels.length-1; i>=0; i--) {
    ...
    if (pt.prevSpread < lvl && signalSpread >= lvl) {
      ...
      _deribitLimitSell(pos.qty).then(dOid => {
        pos._orderPending = false;
        pos.dEntryOid = dOid;
        // Persist entry to DB after order confirmed
...
/**
 * Stop runner, maker-flat Deribit (post_only), wipe sol_live_trades + sol_balance_snapshots, restart.
 * Clears in-memory session so UI stats match DB again.
 */
async function freshReset(startOpts) {
  const opts = startOpts && typeof startOpts === 'object' ? startOpts : DEFAULT_LIVE_START_OPTS;
  ...
  return { ok: true, flat, sessionId: _SESSION_ID };
}
...
async function _reconcileOpenPositions() {
  ...
  try {
    orphans = await SolLiveTrade.findAll({
      where: { type: 'ENTRY', pnlUsd: null, eventAt: { [Op.gte]: cutoff } },
      order: [['id', 'ASC']],
    });
  } catch (e) {
...
async function start(opts = {}) {
  ...
  console.log('═'.repeat(70));
  console.log(`  Symbol       : ${DERIBIT_SYMBOL} (Deribit, post_only maker limits)`);
  ...
  console.log(`  Qty/zone     : 1 SOL total  (split across 4 levels, each multiple of ${DERIBIT_TRADE_AMOUNT_STEP} SOL)`);
  ...
  try {
    ...
    const todayExits = await SolLiveTrade.findAll({
      where: { type: 'EXIT', eventAt: { [Op.gte]: todayStart } },
      attributes: ['pnlUsd'],
    });
  ...
  await _saveBalanceSnapshot('session_start');
...
function stop(reason) {
  ...
  _saveBalanceSnapshot('session_stop').catch(()=>{});
...
function getState() {
  ...
  return {
    ...
    equity:        _st.equity,         // current runner equity (updated on each close)
    dailyPnl:      _st.dailyPnl,
